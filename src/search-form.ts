...
export function renderSearchFormBlock (dateFrom, dateTo) {
  ...
  if ((dateFrom === null) ||
      (Date.parse(dateFrom) < Date.parse(formatDate(minDate)))
  ) {
  ...
  if (
    ...
    const defaultDateTo = new Date(defaultDateFrom.setDate(defaultDateFrom.getDate() + 2));
      ...
      <fieldset class="search-filedset">
        ...
        <div class="row">
          <div>
            ...
            <input id="check-in-date" type="date" value=${currentDateFrom} min=${formatDate(minDate)} max="${formatDate(maxDate)}" name="checkin" />
          </div>
          <div>
            <label for="check-out-date">Дата выезда</label>
            <input id="check-out-date" type="date" value=${currentDateTo} min=${formatDate(minDate)} max="${formatDate(maxDate)}" name="checkout" />
          </div>
          <div>
